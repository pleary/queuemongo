...
QueueMongo.prototype.popItem = function(sortBy, queueItemCallback, callback) {
  ...
  var q = this;
  this.db.collection(this.queueName).count({ status: 'in-progress' }, function(err, count) {
    if(err || count > 0) {
...
QueueMongo.prototype.pushItem = function(doc, callback) {
  var docToInsert = _.extend(doc, { status: 'pending', timestamp: new Date() });
  if(docToInsert['type'] === undefined) docToInsert['type'] = 'default';
  this.db.collection(this.queueName).insert(docToInsert, function(err, docs) {
...
QueueMongo.prototype.updateItemStatus = function(doc, status, callback) {
  ...
  this.db.collection(this.queueName).update({ _id: docToInsert['_id'] }, docToInsert, function(err, doc) {
    if(err) return callback(err);
